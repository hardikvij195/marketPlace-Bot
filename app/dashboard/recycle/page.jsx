...
export default function RecyclePage() {
  ...
  const [pendingId, setPendingId] = useState(null); // id we want to delete
  ...
  const [recycletats, setrecycletats] = useState({
    totalInvoice: 0,
    totalAmount: 0,
    conmissionearned: 0,
  });
                  ...
                  {filteredRecycleData.map((row) => (
                    <tr key={row?.id} className="border-b border-gray-100">
                      <td className="py-3 px-4">
                        {row?.name === "leads" ? row?.name : KeyNums[row?.name]}
                      </td>
